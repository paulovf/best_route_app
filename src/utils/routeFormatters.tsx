...
import { 
  Bus, Plane, Car, Ship, Train, Smartphone, 
  MapPin, Home, Anchor 
} from 'lucide-react';
import { TransportType, LocationType } from '@/types/route';

export const getTransportIcon = (type: TransportType, className = "w-4 h-4") => {
  switch (type) {
...
export const getLocationIcon = (type: LocationType, className = "w-4 h-4") => {
  switch (type) {
    ...
    case 'bus_station': return <Bus className={className} />;
    case 'train_station': return <Train className={className} />;
    case 'boat_station': return <Anchor className={className} />;
