...
import { getTransportIcon, getTransportTypeLabel, getLocationIcon, formatDuration, formatPrice } from '@/utils/routeFormatters';
    ...
    <div className="relative mt-6 pl-8 space-y-6 animate-fadeIn">
      {sortedSteps.map((step, index) => (
        <div key={`${step.order}-${index}`} className="relative">
          ...
          <div>
            <div className="text-sm font-bold text-primary-900 balance">
              {step.origin_departure} <span className="text-neutral-800/40 font-normal mx-1">➔</span> {step.destination_arrival}
            </div>
            
            <p className="text-xs text-neutral-800/50 mt-0.5">
              {step.origin_city} ({step.origin_state}) até {step.destination_city} ({step.destination_state})
            </p>
            
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-neutral-800/60">
              ...
              <span>•</span>
              <span className="font-semibold text-primary-700">{formatPrice(step.average_amount)}</span>
            </div>
          </div>
        </div>
      ))}
