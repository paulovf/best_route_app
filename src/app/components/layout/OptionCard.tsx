...
import { OptionCardStep } from './OpyonCard/Step';
...
export const OptionCard = ({ option, onSelect }: OptionCardProps) => {
  ...
  const getBadgeDetails = (order: number, desc: string) => {
    if (order === 1) return { label: 'RECOMENDADA', css: 'bg-primary-500 text-white' };
    if (desc.toLowerCase().includes('direta') || order === 2) return { label: 'MAIS RÁPIDA', css: 'border border-primary-500 text-primary-600' };
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4" onClick={(e) => {
        e.stopPropagation();
        setIsOpen(!isOpen);
      }}>
        <div>
          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold tracking-wide ${badge.css}`}>
            {badge.label}
          </span>
          <div className="mt-3 flex flex-col items-start gap-1">
            ...
            <div className="text-sm text-neutral-800/60 font-medium">
              {option.total_kilometers} km • {formatPrice(option.total_amount)}
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          className={`w-8 h-8 rounded-full border border-neutral-200 flex items-center justify-center transition-transform duration-300 ${
            isOpen ? 'rotate-180 bg-primary-50 border-primary-500 text-primary-600' : 'text-neutral-800'
          }`}
        >
