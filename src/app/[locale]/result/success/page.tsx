...
export default function SuccessPage() {
  ...
  return (
    <main className="min-h-screen bg-neutral-50 animate-fadeIn">
      <Topbar show={true} />
      <div className="mt-14 max-w-3xl mx-auto px-4 py-8 md:py-12">
        <header className="flex items-center gap-4 mb-6">
          ...
          <div>
            <div className="flex flex-row items-center gap-x-1">
              <h1 className="text-lg font-bold text-primary-700">{routeData.origin_city} ({routeData.origin_state})</h1>
              <MoveRight size={16} />
              <h1 className="text-lg font-bold text-primary-700">{routeData.destination_city} ({routeData.destination_state})</h1>
              <Dot size={26} />
              <h1 className="text-lg font-bold text-neutral-700">{formatDate(routeData.travel_date)}</h1>
            </div>
        ...
        <div className="mb-6 flex items-start gap-2.5 p-3.5 bg-neutral-100 rounded-xl border border-neutral-200/60">
          ...
          <p className="text-xs text-neutral-600 font-medium leading-relaxed">
            <span className="font-bold text-neutral-700">{t("note")}</span> {t("disclaimer")}
          </p>
        </div>

        <div className="space-y-4">
          {[...routeData.options].sort((a, b) => a.order - b.order).map((opt) => (
            <OptionCard key={opt.order} option={opt} />
          ))}
        </div>

        <div className="mt-10">
          <Link href="/" className="flex items-center justify-center w-full bg-neutral-700 text-neutral-50 rounded-full font-semibold h-12 hover:opacity-50 transition-all">
            {t("newSearch")}
          </Link>
        </div>
      </div>
