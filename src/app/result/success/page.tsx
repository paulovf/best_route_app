...
export default function SuccessPage() {
  ...
  const [selectedOrder, setSelectedOrder] = useState<number | null>(sortedOptions[0]?.order ?? null);
  ...
  return (
    <main className="min-h-screen bg-neutral-50 animate-fadeIn">
      <Topbar show={true} />
      <div id="result-screen" className="mt-14 max-w-3xl mx-auto px-4 py-8 md:py-12">
        <header className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-lg font-bold text-primary-700 leading-tight">
              {data.origin_city} ({data.origin_state}) → {data.destination_city} ({data.destination_state}) • <span className="text-neutral-700">{formatDate(data.travel_date)}</span>
            </h1>
